...
//$obj = new Parents();
//$res = $obj->getStudentByParentId($parent_id);
...
//                alert(theResponse);
                if(theResponse != 'no records')
                {
                    ...
                    $('#assignments_tbl').append("<tr><td>"+val.title+"</td><td>"+val.Marks+"</td><td><a href=view_assignment_mark.php?assignment_mark_id="+val.Assignment_mark_id+" rel='facebox' class='btn btn-success'><i class='icon-view icon-white'></i> &nbsp;View</a></td></tr>");
                    });                    
                }
                else
                {
                    $('#assignments_tbl').html('');
                    $('#assignments_tbl').html('<tr class="odd"><td class="dataTables_empty" valign="top" colspan="3">No data available in table</td></tr>');
...
//              $student = $_SESSION['cur_user'];
              $objct_course = new Parents();
              $result = $objct_course->getCoursesByStudent($student_id);
...
<div id="DataTables_Table_0_wrapper" class="dataTables_wrapper" role="grid" id="transcript">
  ...
  <thead>
      <tr role="row">
          ...
          <th aria-label="Name: activate to sort column ascending" style="width: 177px;" colspan="1" rowspan="1" aria-controls="DataTables_Table_0" tabindex="0" role="columnheader" class="sorting">Options</th><!--
...
<tbody id="assignments_tbl" aria-relevant="all" aria-live="polite" role="alert" >
        ...
        <tr>
<!--             <a href="#">
                 <button class="pull-right btn btn-info" onclick="printDiv('transcript');">
