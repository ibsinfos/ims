	...
	<div class="box span12">
		...
		<div class="box-content">
            ...
            <ul class="nav nav-tabs" id="myTab">
                ...
                <li><a href="#create">Student Feedback</a></li>
            </ul>
            ...
            <select name="course" id="course">
              ...
              $result = $objct_course->getCourseByLecturer($teacher);
             while($row = mysql_fetch_array($result)) {
              
              ?>  
                
                <option <?php if( isset($_POST['course']) &&  $_POST['course'] == $row['course_id']){ echo 'selected';} ?> value="<?php echo $row['course_id'];?>"><?php echo $row['course_category_name']."-".$row['course_name']; ?></option>
                <?php } ?>  
                </select>
                <br/>
                <br/>
...
<?php if(isset($_POST['action']) && $_POST['action']=="get_lecture_hours"){
    ...
    $end = $_POST['end_date'];
    $course = $_POST['course'];    
    $summery_result = $obj_user->getLecturerHoursSummery($start, $end, $course);
    $done_hours = 0;
    $expected_hours = 0;
    $total_class = 0;
    $classes_done = 0;
...
<tbody id="lecture_hours_tbl" aria-relevant="all" aria-live="polite" role="alert">
    ...
    <br/><br/><br/>
            ...
            <label class="control-label" for="typeahead">Course</label>
            <div class="controls">
            <select name="feedback_course" id="course">
                <option></option>
              <?php
              $result = $objct_course->getCourseByLecturer($teacher);
             while($row = mysql_fetch_array($result)) {
              ...
              ?>  
                ...
                <option <?php if( isset($_POST['feedback_course']) &&  $_POST['feedback_course'] == $row['course_id']){ echo 'selected';} ?> value="<?php echo $row['course_id'];?>"><?php echo $row['course_category_name']."-".$row['course_name']; ?></option>
                <?php } ?>  
                </select>
            </div>
            </div>
        ...
        $course_id = $_POST['feedback_course'];
        $feedback_res = $obj_user->getLecturerFeedbackRating($feedback_start,$feedback_end,$course_id);
        $feedback_course = $obj_user->getCourseFeedbackRating($feedback_start,$feedback_end,$course_id);
        $obj_course = new Course();
