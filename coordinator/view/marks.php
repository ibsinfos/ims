...
<div class="row-fluid">
	<div class="box span12">
        ...
        <fieldset>
            ...
            <div class="controls">
                <select name="course_id" id="course_id" onchange="getStudents(this.value)">
              <option></option>
                    <?php 
              
               $objct_course = new Student();
              $result = $objct_course->getAllCourses();
...
<div id="DataTables_Table_0_wrapper" class="dataTables_wrapper" role="grid" id="transcript">
  ...
  <thead>
      <tr role="row">
          <th aria-label="Photo: activate to sort column ascending" style="width: 150px;" colspan="1" rowspan="1" aria-controls="DataTables_Table_0" tabindex="0" role="columnheader" class="sorting" width="80">Student Name </th>
          <th aria-label="Name: activate to sort column ascending" style="width: 177px;" colspan="1" rowspan="1" aria-controls="DataTables_Table_0" tabindex="0" role="columnheader" class="sorting"></th>
  </thead>   
<tbody id="students_tbl" aria-relevant="all" aria-live="polite" role="alert" >
</tbody>
</table>
</div>
    </div>
...
//                            var course_id = $('#course').val();
                       ...
                       cache:false,
                       async:false,
                       data:{get_student_list:true,course_id:course_id},
                       success:function(theResponse){
                           
                           if(theResponse != 'no records'){
//                               alert(theResponse);
                               var course_id = $.parseJSON(theResponse);
                               $('#students_tbl').html('');
                               $.each(course_id,function(i,val){
//                                   alert(val.student_id);
                                   $('#students_tbl').append("<tr><td>"+val.first_name+" "+val.last_name+"</td><td><a href=view_assignment_marks.php?student_id="+val.student_id+"&course="+val.course_id+" class='btn btn-success' rel='facebox'><i class='icon-view icon-white'></i>View</a></td></tr>");                                   
                               });
