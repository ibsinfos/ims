...
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
	...
	<meta charset="utf-8">
	<title>Course Materials | KEY - Institute Management System</title>
...
//          alert(course_name);
          ...
          $.ajax({
            ...
            cache: false,
            async:false,
            data: {assignment:true,course:course},
            success: function(theResponse){ 
                if(theResponse != 'no records')
                {
                    ...
                    $('#assignments_tbl').append("<tr><td>"+val.title+"</td><td>"+val.uploaded_date+"</td><td>"+val.due_date+"</td><td><a href=view_assignment.php?assignment_id="+val.assignment_id+" rel='facebox' class='btn btn-success'><i class='icon-view icon-white'></i> View</a></td><td>&nbsp;</td></tr>");
                    }); 
          ...
          $.ajax({
            ...
            cache: false,
            async:false,
            data: {lessons:true,courses:courses},
            success: function(theResponse){              
//                 alert(theResponse);
                if(theResponse != 'no records')
                {
                    ...
                    $('#lessons_tbl').append("<tr><td>"+val.date+"</td><td><a href=view_lessons.php?lesson_id="+val.tbl_lesson_id+" rel='facebox'class='btn btn-success'><i class='icon-view icon-white'></i> View</a></td></tr>");
                    }); 
                     $('a[rel*=facebox]').facebox({});
                }
                else
...
<div id="DataTables_Table_0_wrapper" class="dataTables_wrapper" role="grid">
  ...
  <thead>
      <tr role="row">
          ...
          <th aria-label="Name: activate to sort column ascending" style="width: 177px;" colspan="1" rowspan="1" aria-controls="DataTables_Table_0" tabindex="0" role="columnheader" class="sorting">Uploaded</th>
...
<tbody id="assignments_tbl" aria-relevant="all" aria-live="polite" role="alert" >
        ...
        <fieldset>
            ...
            <label class="control-label" for="typeahead">Course </label>
            <div class="controls">
            <select name="courses" onchange="getlessons(this.value)" id="courses">
                <option></option>
              <?php 
              $objct_course = new Students();
              $result = $objct_course->getCoursesByStudent($student);
...
<table aria-describedby="DataTables_Table_0_info" id="DataTables_Table_0" class="table table-striped table-bordered bootstrap-datatable datatable dataTable">
  <thead>
      <tr role="row">
          ...
          <th aria-label="Name: activate to sort column ascending" style="width: 177px;" colspan="1" rowspan="1" aria-controls="DataTables_Table_0" tabindex="0" role="columnheader" class="sorting">Lesson</th>
