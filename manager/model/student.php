...
//    die;
    ...
    public function getCoursesByStudent() {
        $sql = "SELECT * FROM tbl_students,tbl_course WHERE FIND_IN_SET(tbl_course.course_id,tbl_students.course)>0";
    ...
    public function getAssignmentMars($course, $student) {
        $sql = "SELECT * FROM tbl_assignment_marks INNER JOIN 
    ...
    public function getYearWiseAdmissionPayments()
    {
        $sql = "SELECT YEAR(payment_date) as year, SUM(amount) as total FROM tbl_payments GROUP BY YEAR(payment_date)";
        $object = new Connection();
        $result = $object->query($sql);
        return $result;
    }
    public function getMonthWiseAdmissionPayments()
    {
        $sql = "SELECT YEAR(payment_date) as year, MONTH(payment_date) as month, SUM(amount) as total FROM tbl_payments GROUP BY YEAR(payment_date), MONTH(payment_date)";
        $object = new Connection();
        $result = $object->query($sql);
        return $result;
    }
    ...
    public function getAllClasSchedulesInYear($year,$course){
       $sql = "SELECT * FROM tbl_schedule WHERE course='$course' AND YEAR(date)='$year'";
        ...
        public function getAttendanceDates($year,$course){
         $sql = "SELECT * FROM tbl_student_attendance INNER JOIN 
             tbl_schedule ON tbl_schedule.schedule_id=tbl_student_attendance.schedule_id WHERE YEAR(date)='$year' AND course='$course'";
          $db = new Connection();
        $result =$db->query($sql);
        return $result;
        }
        
        public function getAllClasSchedulesByMonth($year,$course){
...
//       $sql = "SELECT *,Month(date) as month FROM tbl_schedule WHERE course='$course' AND YEAR(date)='$year'";
       $sql1 = "SELECT Month(tbl_schedule.date) as month,
            ...
            WHERE YEAR(tbl_schedule.date)='$year' 
   ...
   public function getCoursesById($course_id){
    ...
    public function getAttendanceChartDatesInMonth($month,$course){
         $sql1 = "SELECT
            count(CASE WHEN tbl_student_attendance.status='0' THEN tbl_student_attendance.status END) as attended,           
            count(CASE WHEN tbl_student_attendance.status='1' THEN tbl_student_attendance.status END) as late_attended,
            COUNT(tbl_schedule.date) as attendance_count
            ...
            INNER JOIN tbl_schedule 
            ON tbl_schedule.schedule_id=tbl_student_attendance.schedule_id 
            WHERE  MONTH(tbl_schedule.date)='$month' 
            AND tbl_schedule.course='$course'";
        ...
        $results = array( "Attended" => "$attended", "Late attended" => "$late_attended", "Not attended" =>"$not_attended");
